...
def generate_character_conversation():
    ...
    # Generate image topics for the conversation
    ...
    topic_short = topic.replace(' ', '_')[:20]
    ...
    print(f"  • Images: {len(image_topics)} topics")
    print(f"  • Output: {output_path}\n")
    
    # Generate a longer input text for the conversation based on the desired duration
    # Each 10 seconds needs about 25 words, so scale accordingly
    ...
    video_path = generate_conversation_video(
        ...
        # Set image duration to be approximately total duration divided by number of images,
        # but with a reasonable minimum and maximum
        image_duration=max(5, min(15, args.duration // len(image_topics)))
    )
